...
export function rewriteContextNamespaceCalls(payload: unknown): unknown {
	if (!isRecord(payload) || !Array.isArray(payload.input)) return payload;
	let changed = false;
	const input = payload.input.map((item) => {
		if (!isRecord(item) || item.type !== "function_call") return item;
		if ((item.namespace !== "history" && item.namespace !== "notes") || item.name !== item.namespace) return item;
		if (typeof item.arguments !== "string") return item;
		let arguments_: unknown;
		try {
			arguments_ = JSON.parse(item.arguments) as unknown;
		} catch {
			return item;
		}
		if (!isRecord(arguments_) || typeof arguments_.action !== "string" || !ACTIONS[item.namespace].has(arguments_.action)) return item;
		const { action: _action, ...rest } = arguments_;
		changed = true;
		return { ...item, name: arguments_.action, arguments: JSON.stringify(rest) };
	});
...
/**
 * Pi's native provider parser preserves Responses namespace calls, but the
 * agent loop resolves tools by ToolCall.name. Route the parsed assistant
 * message at the public message_end boundary before tool execution begins.
 */
export function routeContextNamespaceToolMessage(message: AgentMessage): AgentMessage {
	if (message.role !== "assistant") return message;
	let changed = false;
	const content = message.content.map((block) => {
		if (block.type !== "toolCall") return block;
		const routed = routeContextNamespaceToolCall(block);
		changed ||= routed !== block;
		return routed;
	});
